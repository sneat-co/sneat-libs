...
export abstract class HappeningBasePage extends CalendarBasePage {
  ...
  // private calendariusSpaceDbo?: ICalendariusSpaceDbo | null;
  private calendariusSpaceSub?: Subscription;
  ...
  protected override onSpaceIdChanged(): void {
    ...
    this.calendariusSpaceSub?.unsubscribe();
    this.calendariusSpaceSub = this.params.calendariusSpaceService
      ...
      .subscribe({
        next: (calendariusSpace) => {
          ...
          if (
            happening?.id &&
            !happening?.dbo // If we loaded happening record we use it as a brief and ignore the brief from calendariusSpace
          ) {
            const brief =
              calendariusSpace.dbo?.recurringHappenings?.[happening?.id];
            if (brief) {
              this.setHappening(
                { ...happening, brief },
              );
