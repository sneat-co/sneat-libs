...
**Phase 3 — Never-extracted in-repo contracts.** commitius, communitycentrum, togethered extract directly into the monorepo (skipping the `ext-*` generation); their product repos switch `workspace:*` → published versions. circleus and sneatclub follow the founder decisions recorded 2026-08-26 (see Decisions): circleus's scaffold stub is PROMOTED via the monorepo's contract generator — `libs/circleus` is scaffolded in `sneat-ext-contracts`, the template-derived API is ported from the `ext-circleus` stub with Template→Circleus token renames, `@sneat/extension-circleus-contract` first-publishes, `ext-circleus` is archived (its mispublishing workflow is already disabled), and the circleus product repo is wired to the published package. sneatclub's in-repo "contract" turned out to be a vendored, renamed copy of `@sneat/extension-template-contract@0.1.0` build output rather than an authored contract, so a real contract is AUTHORED in `sneat-ext-contracts` as `libs/sneatclub`, derived from that vendored copy's `.d.ts` surface (the intended API), first-published as `@sneat/extension-sneatclub-contract`; sneat-club then swaps off the vendored lib and deletes it.
...
- **circleus contract: PROMOTE via the monorepo's contract generator** — founder decision 2026-08-26. `libs/circleus` is scaffolded in `sneat-ext-contracts` using the monorepo's contract generator; the template-derived API is ported from the `ext-circleus` stub with Template→Circleus token renames; `@sneat/extension-circleus-contract` first-publishes from there; `ext-circleus` is archived (its mispublishing workflow is already disabled); the circleus product repo is then wired to the published package.
- **sneatclub contract: AUTHOR a real contract** — founder decision 2026-08-26. sneat-club's in-repo "contract" was found to be a vendored, renamed copy of `@sneat/extension-template-contract@0.1.0` build output — a placeholder per its own commit message, not an authored contract. The real contract is authored in `sneat-ext-contracts` as `libs/sneatclub`, derived from the vendored copy's `.d.ts` surface (which is the intended API), and first-published as `@sneat/extension-sneatclub-contract`; sneat-club then swaps off the vendored lib and deletes it.
- **Archiving gate: zero-reference, direct AND indirect** — founder decision 2026-08-27. Superseded ext-* repositories are archived only on a zero-reference gate, proven by an org-wide `go.mod` re-audit at campaign close covering both direct and indirect references; the fleet-wide `sneat-core-modules` ≥v0.65.13 bump wave executes the same session (2026-08-27) to clear inherited indirect references, tracked as backstage task `fleet-core-modules-bump`.
