...
| [Per-extension contract repo (`ext-<name>`) convention](extension-contract-repo/README.md) | Deprecated | The convention for extracting each extension's public contract surface into a dedicated, dependency-light `ext-<name>` repo (polyglot `backend/` + `frontend/`), with the zero-other-extension-deps invariant, the ownership test that decides what lives there, the two cross-extension interaction directions, naming, and enforcement. |
